...
%
%PR refers to Polak-Ribere update to the non-linear conjugate gradient
%method
%----------------------------------------------------------------------------------


function [x_opt,f_opt] = PR_rosenbrock(f, Df, x_init, max_iter, change_tol_conj_grad_rosenbrock, alpha_0, alpha_1, max_iter_secant, change_tol_secant)
...
if g == 0
    ...
    while iter<= max_iter
        ...
        beta = (Df(x)'*(Df(x)-Df(x_curr))/(Df(x_curr)'*Df(x_curr)));
        d = beta*d-g;
